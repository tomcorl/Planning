...
// Reserved negative identifiers for synthetic "pending" rows.
// They cannot collide with real team IDs generated by the database.
const PENDING_EQUIP_BASE = -1000001;
...
function getPendingEquipeId(companyIndex, pendingIndex) {
  return PENDING_EQUIP_BASE - companyIndex * PENDING_EQUIP_STRIDE - pendingIndex;
}
...
export default function App() {
  ...
  const pendingCompanyByEquipeId = useMemo(() => {
    const map = new Map();
    companies.forEach((comp, companyIndex) => {
      const pendingCount = getPendingCountForCompany(comp);
      for (let p = 0; p < pendingCount; p++) {
        map.set(getPendingEquipeId(companyIndex, p), comp.id);
      }
    });
    return map;
  }, [companies]);
  ...
  const gridRows = useMemo(() => {
    ...
    for (let c = 0; c < companies.length; c++) {
      ...
      const pendingCount = getPendingCountForCompany(comp);
      for (let p = 0; p < pendingCount; p++) {
        rows.push({ type: 'pending', id: `p-${c}-${p}`, equipeIndex: getPendingEquipeId(c, p) });
      }
  ...
  function endSelection() {
    ...
    setForm({
      id: null,
      company_id: teamById.get(selection.equipe)?.companyId || pendingCompanyByEquipeId.get(selection.equipe) || companies[0]?.id,
  ...
  function saveModal() {
    ...
    commit(() => {
      if (modal.type === 'chantier') {
        ...
        const companyId = form.company_id || teamById.get(equipe)?.companyId || pendingCompanyByEquipeId.get(equipe) || companies[0]?.id;
