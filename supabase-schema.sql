...
-- RPC pour marquer la migration equipe_id comme terminée
...
BEGIN
  ...
  FOR comp_id IN SELECT id FROM companies LOOP
    ...
    DELETE FROM equipes WHERE company_id = comp_id AND id NOT IN (
      ...
      WHERE (x->>'company_id') = comp_id AND (x->>'id') IS NOT NULL AND (x->>'id') ~ '^[0-9]+$'
    );
    INSERT INTO equipes (id, company_id, nom, ordre)
    SELECT (x->>'id')::INT, (x->>'company_id')::TEXT, (x->>'nom')::TEXT, COALESCE((x->>'ordre')::INT, 1)
    FROM jsonb_array_elements(p_equipes) AS x
    WHERE (x->>'company_id') = comp_id AND (x->>'id') IS NOT NULL AND (x->>'id') ~ '^[0-9]+$'
    ON CONFLICT (id) DO UPDATE SET
      company_id = EXCLUDED.company_id, nom = EXCLUDED.nom, ordre = EXCLUDED.ordre;
    INSERT INTO equipes (company_id, nom, ordre)
    SELECT (x->>'company_id')::TEXT, (x->>'nom')::TEXT, COALESCE((x->>'ordre')::INT, 1)
    FROM jsonb_array_elements(p_equipes) AS x
    WHERE (x->>'company_id') = comp_id AND ((x->>'id') IS NULL OR NOT (x->>'id') ~ '^[0-9]+$');
...
-- ============================================================
-- MIGRATION : equipe_id (run once on existing databases)
-- ============================================================
...
-- SELECT column_name, is_identity, identity_generation FROM information_schema.columns WHERE table_name = 'equipes' AND column_name = 'id';
